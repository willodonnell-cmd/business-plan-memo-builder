...
const retiredApproverNames = ["Carter Andrus"];
...
export async function getWorkspacePlan(): Promise<WorkspacePlan> {
  ...
  const questions = await d1
    .prepare(
      "SELECT id, section_id, author_name, author_email, author_role, visibility, status, issue_type, function_name, body, response FROM section_questions WHERE plan_id = ? ORDER BY created_at DESC",
    )
    .bind(PLAN_ID)
    .all<QuestionRow>();
...
export async function createQuestion(input: {
  ...
  await d1
    .prepare(
      "INSERT INTO section_questions (id, plan_id, section_id, author_name, author_email, author_role, visibility, status, issue_type, function_name, body, response, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    )
    .bind(
      crypto.randomUUID(),
      PLAN_ID,
...
async function ensureSchema(d1: D1Database) {
  const statements = [
    ...
    "CREATE TABLE IF NOT EXISTS section_questions (id text PRIMARY KEY NOT NULL, plan_id text NOT NULL, section_id text NOT NULL, author_name text NOT NULL, author_email text DEFAULT '' NOT NULL, author_role text NOT NULL, visibility text DEFAULT 'Public' NOT NULL, status text DEFAULT 'Open' NOT NULL, issue_type text DEFAULT 'Clarification' NOT NULL, function_name text DEFAULT '' NOT NULL, body text NOT NULL, response text, created_at integer NOT NULL, updated_at integer NOT NULL, FOREIGN KEY (plan_id) REFERENCES business_plans(id) ON DELETE cascade, FOREIGN KEY (section_id) REFERENCES memo_sections(id) ON DELETE cascade)",
  ...
  await addColumnIfMissing(d1, "section_questions", "author_email", "text DEFAULT '' NOT NULL");
...
async function ensureApproverDefaults(d1: D1Database) {
  ...
  for (const name of retiredApproverNames) {
    if (existingNames.has(name)) {
      statements.push(
        d1.prepare("DELETE FROM approvers WHERE plan_id = ? AND name = ?").bind(PLAN_ID, name),
      );
  ...
  statements.push(
    d1
      ...
      .bind("Questions", now, PLAN_ID, "Reviewing", "Needs answers", "Supportive", "Needs Revision", "Approved with Conditions", "Not Approved"),
  );
